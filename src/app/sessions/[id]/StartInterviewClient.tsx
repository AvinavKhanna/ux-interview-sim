...
import type { Knobs } from '@/lib/prompt/personaPrompt';
...
export default function StartInterviewClient({ sessionId, persona, project }: StartInterviewClientProps) {
  ...
  const requestTokenAndStart = useCallback(async () => {
    try {
      ...
      if (data?.persona) setPersonaState(data.persona as PersonaSummary);
      if ('project' in data) setProjectState((data.project as ProjectSummary) ?? null);
...
function LiveInterview({
  ...
  const computeBaselineKnobs = useCallback((): Knobs => {
    const base = initialKnobs ?? deriveInitialKnobs(persona ?? {});
    return {
      guardedness: clamp(base.guardedness),
      hesitation: clamp(base.hesitation),
      talkativeness: clamp((base.talkativeness ?? 0.5) - 0.15),
      openness: clamp(base.openness),
    };
  }, [initialKnobs, persona]);
  ...
  const enqueueAudio = useCallback(async (blob: Blob | null) => {
  ...
  const sendPromptUpdate = useCallback(async () => {
    const prompt = buildPrompt(personaRef.current ?? {}, projectRef.current ?? null, knobsRef.current);
    try {
      const maybe = clientRef.current?.sendSessionSettings?.({ systemPrompt: prompt, customSessionId: sessionId });
  ...
  const handleUserTurn = useCallback(
    ...
    [coachOn, ensureLastHume, requestCoachCue]
  );
    ...
    <Card className="space-y-5 p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <div className="text-xs text-gray-500">Status: {conn}</div>
        </div>
