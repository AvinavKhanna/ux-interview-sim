...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!report) return <ReportPoll id={id} />;
...
function ReportPoll({ id }: { id: string }) {
  'use client';
  const React = require('react') as typeof import('react');
  ...
  const { talkTimeRatio: ttr, openVsClosed: ovc, summary: sum } = require('@/lib/analysis/interview') as typeof import('@/lib/analysis/interview');
  const tt = ttr(report.turns);
  const oc = ovc(report.turns);
  const lines = sum(report.turns);
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <header className="flex items-center justify-between">
        <div>
      ...
      <section className="rounded border p-4">
        ...
        {(!report.turns || report.turns.length === 0) ? (
          ...
          // @ts-expect-error Client component within client render
          <Transcript turns={report.turns} />
        )}
      </section>
    </div>
